<script lang="ts">
	...
	export let seed: number = Math.ceil(Math.random() * 9999);
	let renderSize = 2 ** 10;
	...
	export function flowers() {
		...
		const getCloverVariantConfig = (): Omit<
			CuteCirclyFlower,
			'draw' | 'grow' | 'age' | 'entityRotater' | 'lifeStatus' | 'petals' | 'petalSize'
		> => {
			let gridSpacing = 50;
		...
		const generateFlowerSpeciesConfig = ({
			...
			const speciesFeatures = {
				centerColor: Math.random() > 0.5 ? 'white' : '#ffffcc',
		...
		function drawCloverLeafHalf(
			...
			const controlPoint1b: Coordinates = entityRotater([x + size, y + length * 0.75]);
			const controlPoint2a: Coordinates = entityRotater([x + size, y + leafBulge]);
			const controlPoint2b: Coordinates = entityRotater([x - size, y + leafBulge]);
			const corner2: Coordinates = entityRotater([x - size, y + length]);
			const controlPoint3a: Coordinates = entityRotater([x - size, y + length * 0.75]);
...
{#key seed}
	<canvas bind:this={canvas} width={renderSize} height={renderSize}>Rendering…</canvas>
	<p>Refresh, to generate a new field 🌻.</p>
{/key}
